...
#import "UITextField+Separator.h"
...
@implementation UITextField (Separator)
...
-(BOOL)dw_ShouldChangeCharactersInRange:(NSRange)range replacementString:(NSString *)string
{
    ...
    //获取初始光标位置，为了后面还原
    NSInteger curP = [self cursorPosition];
    //判断是否在删除字符操作
    BOOL isDeleting = (range.length == 1 && [string isEqualToString:@""]);
    ///获取目标字符串
...
//    string = [self absoluteNoSeperatorString:string input:!range.length];
    string = [string stringByReplacingOccurrencesOfString:self.componentsSeparator withString:@""];
    ...
    ///强行改变textField
    self.text = string;
    //上面强行设置text后会改变光标位置，下面还原一下
    if (isDeleting) {
        NSInteger p = curP-1;
        if (p > string.length) p = string.length;
        [self setCursorPosition:p];
    } else {
        if (curP < string.length-2) {
            [self setCursorPosition:curP+1];
        }
    }
...
//    return [self absoluteNoSeperatorString:self.text input:YES];
    return [self.text stringByReplacingOccurrencesOfString:self.componentsSeparator withString:@""];
}
